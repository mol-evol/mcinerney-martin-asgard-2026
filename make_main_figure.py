...
Main-text Figure 1 (two panels) for the Matters Arising.
  a: Asgard:Alphaproteobacteria ratio vs minimum candidate count per tuple --
     the standard (occupancy-weighted) ratio collapses onto the stable head-to-head
     value (the denominator effect).
  b: Asgard:Alphaproteobacteria aELW ratio across the four deposited soft-core
     cut-offs (s0=no filter, s10, s25, s67) -- the filter effect; the ratio inverts
     to 0.44 when the filter is removed.
These are panels c and e of the full Extended Data Fig. 1, drawn from the same
deposited data (Zenodo 15048010). Usage: python make_main_figure.py [DATA_DIR] [OUT_DIR]
"""
import os, sys
...
A.plot(mc, h2h, 's-', color=CC, lw=1.3, ms=4, mec='white', mew=0.6, label='Head-to-head c-ELW ratio', zorder=3)
...
A.legend(loc='upper right', frameon=False, fontsize=7); letter(A, 'a')
...
B.set_ylabel('Asgard : Alphaproteobacteria aELW ratio'); nospine(B); letter(B, 'b')
