...
def part5_kegg(r):
    ...
    # v15: the default sort is unstable and 24.5%% of queries have their maximum Prob
    # tied across >1 distinct target, so this assignment was not reproducible.
    # kind='mergesort' is stable: the tie-break is the last occurrence in file order.
    best = ann.sort_values("Prob", kind="mergesort").drop_duplicates("Query", keep="last")
...
def part6_figure(sweep, r, k, flip, ad):
    """Extended Data Fig. 1 -- five panels (a-e) from this one pipeline (v15 revision).
    a: candidate-set inclusion vs aELW (standard + conditional);  b: standard vs conditional
    ratio for six bacterial candidates;  c: Asgard:Cyano ratio vs minimum candidates per tuple;
    d: within-gene donor flip;  e: tree bacterial depletion.
    Removed at revision: the Asgard:Alpha stratification and the filter sweep (both are now
    main-text Fig. 1a,b and were duplicated here), and the Asgard-monophyly panel (Asgard
    paraphyly is expected if eukaryotes branch within Asgard, so the panel did not test what
    its placement implied).  Saves figure_asgard_full.svg / .pdf / .png (true vector)."""
    ...
    gs=GridSpec(3,2,figure=fig,hspace=0.46,wspace=0.34,left=0.085,right=0.975,top=0.97,bottom=0.06)
    A=fig.add_subplot(gs[0,0]);B=fig.add_subplot(gs[0,1]);C=fig.add_subplot(gs[1,0])
    Dx=fig.add_subplot(gs[1,1]);E=fig.add_subplot(gs[2,0])
    ...
    # v15: full clade names in panel a (referee request)
    lt={'Asgard':'Asgardarchaeota','Cyanobacteriota':'Cyanobacteriota','Betaproteobacteria':'Betaproteobacteria','Alphaproteobacteria':'Alphaproteobacteria','Actinomycetota':'Actinomycetota','Gammaproteobacteria':'Gammaproteobacteria','Myxococcota':'Myxococcota'}
    ...
    B.bar(xp+w/2,h2h['h2h_ratio'],w,color=CC,edgecolor='white',linewidth=0.5,label='Conditional c-ELW ratio')
    ...
    def stratplot(ax,data,tgt,ylim,letterc):
        ...
        ax.plot(data['min_c'],data['h2h_ratio'],'s-',color=CC,lw=1.3,ms=4,mec='white',mew=0.6,label='Conditional c-ELW ratio',zorder=3)
    ...
    stratplot(C,pdd,'Cyano',(0.5,5),'c')   # v15: Asgard:Alpha stratification moved to main Fig. 1b
    # d -- within-gene flip
    ...
    for col,c,lab in [('to_Bacteria',CC,'to Bacteria'),('to_Asgard',CS,'to Asgard'),('to_Archaea','#7f8c8d','to Archaea')]:
        vals=np.array([fl.loc[g,col] for g in gB]); Dx.bar(range(len(gB)),vals,bottom=bott,color=c,label=lab)
        ...
        bott=bott+vals
    Dx.set_xticks(range(len(gB))); Dx.set_xticklabels([("Asgard" if g=="Asgard" else "Bacteria")+" call\n(s10)" for g in gB]); Dx.set_ylabel('re-assignment, filter OFF (%)'); Dx.set_ylim(0,100)
    Dx.legend(frameon=False,fontsize=6.5,loc='lower center'); nospine(Dx); letter(Dx,'d'); Dx.set_title('Same genes, filter off',fontsize=8.5,loc='left',pad=2)
    # e -- tree bacterial depletion
    vals=[kk.loc[c,"pct_no_bacteria"] for c in cats]; E.bar(range(len(cats)),vals,color='#16a085'); E.axhline(nobact,ls='--',c='grey',lw=0.8)
    ...
    E.set_xticks(range(len(cats))); E.set_xticklabels(cats,rotation=12,fontsize=7); E.set_ylim(0,100); E.set_ylabel('% Asgard-winner trees\nwith NO bacterium'); nospine(E); letter(E,'e'); E.set_title('Filter empties trees of bacteria',fontsize=8.5,loc='left',pad=2)
    for ext in ('svg','pdf','png'):
        fig.savefig(os.path.join(HERE,f"figure_asgard_full.{ext}"),dpi=300 if ext=='png' else None,bbox_inches='tight')
    print("\n  figure written (5 panels a-e): figure_asgard_full.svg / .pdf / .png")
...
def part9_stemlength():
    ...
    a["Prob"]=pd.to_numeric(a["Prob"],errors="coerce"); b=a.sort_values("Prob",kind="mergesort").drop_duplicates("Query",keep="last")  # stable: see PART 5 note
...
def _stem_figure(win,donors,perdon):
    ...
    B.set_title("One donor, one event: spread is unexplained\nby timing or by adaptive distance",fontsize=8,loc='left',pad=3); nospine(B); letter(B,'b')
...
if __name__ == "__main__":
    ...
    SUMMARY["caveats"] = {
        ...
        "aELW_absolute": ("This pipeline reproduces the published RATIO (7.74), occupancy (63.8/19.7%), "
               ...
               "Asgard advantage. Extended Data Fig. 1 (panels a-e) and the stem figure are produced by this one script.")}
